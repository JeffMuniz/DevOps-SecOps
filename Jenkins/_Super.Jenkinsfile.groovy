...
pipeline {
    ...
    environment {
        ...
        PRJ_PATH='.'
        PUBLISH_PATH='app'
        PRJ_NAME='macna.SincronizadorCapeante.sln'
        APPLICATION_NAME='fature-auditado-sync-capeante'
        OPENSHIFT_PROJECT_NAME='fature-auditado'
        TEST_PATH='./tests/macna.Sincronizador.Capeante.Tests'
        
        //CONFIGMAP_DEV="src/macna.Queue.MotorContingencia.Cassi.Worker/appSettings-dev.json"
        //CONFIGMAP_HML="src/macna.Queue.MotorContingencia.Cassi.Worker/appSettings-hml.json"
        //CONFIGMAP_PRD="src/macna.Queue.MotorContingencia.Cassi.Worker/appSettings-prd.json"
        ...
        SECRET_FILENAME="src/macna.Sincronizador.Capeante/appsettings.json"
        
        SECRET_SENHA_DATIVA="senha-dativa"
        
        SONAR_PATH='/home/jenkins/.dotnet/tools'       
        SONAR_SERVER='http://sonarqube.macna.local'
        ...
        SONAR_PROJECT_URL='http://sonarqube.macna.local/dashboard?id=sync-capeante'
        
        CHAT_WEBHOOK='https://chat.googleapis.com/v1/spaces/some-token
    }
    options {
    ...
    triggers {
        gitlab(
            ...
            secretToken: 'git-licensed-some-token-keys')
    }
    stages 
    {
        stage ( DEV ) {
            ...
            stages {
                ...
                stage('clean') {
                    ...
                    steps 
                    {
                        ...
                        sh 'dotnet restore -s http://nexus.macna.local/repository/nuget-group/ -s https://api.nuget.org/v3/index.json ${PRJ_PATH}/${PRJ_NAME}'
                    }
                }
                stage('sonar-config') {
                   steps 
                ...
                stage('Sonar-Start') {
                    steps
                    {
                        script 
                        {
                            withSonarQubeEnv('sonarqube macna')  
                            {           
                                echo "QUEM FEZ COMMIT? ${env.gitlabUserName}"
                            ...
                            steps
                            {
                                script
                                {
                                    def handle = triggerRemoteJob(
                                        job: 'http://jenkins-ci.macna.local/job/RunFortify',
                            ...
                            timeout(time:10, unit:'MINUTES') {
                               input message: "Promote TAG ${env.image_tag} to PRD?", ok: "Promote"
                                ...
                                sh """
                                    ...
                                    oc image mirror --insecure docker-registry-default.cpdh.macna.local/${OPENSHIFT_PROJECT_NAME}-hml/${APPLICATION_NAME}:${env.image_tag} docker-registry-default.cppr.macna.local/${OPENSHIFT_PROJECT_NAME}/${APPLICATION_NAME}:${env.image_tag}
                                """
    ...
    post {
        always  {
            script {
                // Para lista completa de cows, acesse: http://cowsay-fature-auditado-dev.cpdh.macna.local/fun.pl?list
                ...
                def response = httpRequest "http://cowsay-fature-auditado-dev.cpdh.macna.local/fun.pl?addmsg=${msg}&uso=chat&cows=${cows}"
