...
Creating a Jenkinsfile, which is checked into source control [1], provides a number of immediate s:
...
As discussed in the Defining a Pipeline in SCM, a Jenkinsfile is
 a text file that contains the definition of a Jenkins Pipeline 
 and is checked into source control. Consider the following Pipeline which implements a basic three-stage continuous delivery pipeline.
...
pipeline {
    ...
    stages {
        ...
        stage('Test From VScode') {
            steps {
                echo 'Testing... Test From VScode'
            }
        }
        stage('Deploy') {
            steps {
...
In this Pipeline example, the credentials assigned to the three COMMON_BITBUCKET_CREDS…​ environment variables are scoped only to Example stage 1, so these credential variables are not available for use in this Example stage 2 stage’s steps. If, however, the environment directive in this Pipeline were moved immediately within the pipeline block (as is the case in the Secret text Pipeline example above), then these COMMON_BITBUCKET_CREDS…​ environment variables would be scoped globally and could be used in any stage’s steps.
Secret files
...
In this Pipeline example, the credentials assigned to the $SSH_KEY_FOR_ABC, $CERTIFICATE_FOR_XYZ and
$XYZ-CERTIFICATE-PASSWORD environment variables are scoped only within their respective withCredentials( …​ ) { …​ } steps, so these credential variables are not available for use in this Example stage 2 stage’s steps.
...
Click Generate Pipeline Script to generate the final withCredentials( …​ ) { …​ } step snippet.
